...
fun broadcastSettings(context: Context) {
   context.sendBroadcast(Intent(Constant.BROADCAST_SETTING_ACTION))
}
...
fun openApplication(context: Context, packageName: String, appType: String = "") {
    ...
    if (appInstalledOrNot(context, packageName)) {
        ...
        if(appType == Constant.ANDROID_DEVICE_POLICY) {
...
fun SwitchMaterial.changeColor(context: Context) {
    val colorRed = ContextCompat.getColor(context, R.color.red)
    val colorRedTra = ContextCompat.getColor(context, R.color.red_transparent)

    val colorGreen = ContextCompat.getColor(context, R.color.green)
    val colorGreenTra = ContextCompat.getColor(context, R.color.green_transparent)
    if (!isChecked) {
        thumbDrawable.setTint(colorRed)
        trackDrawable.setTint(colorRedTra)

    } else {
        thumbDrawable.setTint(colorGreen)
        trackDrawable.setTint(colorGreenTra)
    }
}
...
fun setVerifiedIcon(context: Context, view: TextInputEditText) {
    view.isEnabled = false
    val drawableEnd = ResourcesCompat.getDrawable(
        context.resources, R.drawable.verified, null
    )
    drawableEnd?.let {
        it.setBounds(
            0, 0,
