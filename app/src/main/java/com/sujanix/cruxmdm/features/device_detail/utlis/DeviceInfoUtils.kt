package com.sujanix.cruxmdm.features.device_detail.utlis
...
import com.sujanix.cruxmdm.features.core.data.model.NetworkConnectivity
import com.sujanix.cruxmdm.features.core.utlis.externalMemoryAvailable
import com.sujanix.cruxmdm.features.core.utlis.loadFileAsString
...
val TAG: String = "DeviceInfoUtils"
...
//            val imei = Settings.Global.getString(activity?.contentResolver, "imei")
        ...
        deviceDetails.put("Serial No", serialNo)
...
fun getTotalExternalMemorySize(): Pair<Long, Long>? {
    return try {
        if (externalMemoryAvailable()) {
            ...
            val totalStorage = predictActualTotalStorage(totalBytes / (1024 * 1024 * 1024))
...
fun predictActualTotalStorage(storage: Long): Long {
    return when(storage) {
        in 0L..64L -> { 64 }
        in 65L..128L -> { 128 }
        in 129L..256L -> { 256 }
